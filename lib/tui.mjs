...
import { getClaudeUserId, savePetConfig, loadPetConfig, saveProfile, getProfiles, getActiveProfile, switchToProfile, isHookInstalled, installHook, removeHook, getCompanionName, renameCompanion, getCompanionPersonality, setCompanionPersonality, deleteCompanion } from './config.mjs';
...
function renderStats(stats, colorFn = chalk.white) {
...
function renderProfileCard(bones, profileName, profile, isActive, index, total) {
  const rarityColor = RARITY_CHALK[bones.rarity] || chalk.white;
  ...
  // Sprite
  lines.push('');
  for (const spriteLine of renderSprite(bones, 0)) {
...
function waitForGalleryKey(total) {
  return new Promise(resolve => {
    ...
    function onKey(ch, key) {
      ...
      if (key.name === 'left' && total > 1) action = 'prev';
      else if (key.name === 'right' && total > 1) action = 'next';
      else if (key.name === 'return') action = 'apply';
      else if (key.name === 'escape' || (key.ctrl && key.name === 'c')) action = 'cancel';
...
export async function runSwitch() {
  ...
  function drawCard() {
    ...
    const lines = renderProfileCard(galleryBones[index], galleryNames[index],
      galleryProfiles[index], isActiveEntry(index), index, galleryNames.length);
  ...
  while (true) {
    const action = await waitForGalleryKey(galleryNames.length);
    ...
    if (isActiveEntry(index)) continue;
    ...
    // Switching to default = restore original salt, clear active profile
    if (name === DEFAULT_PROFILE_NAME) {
      const cfg = loadPetConfig();
      cfg.salt = ORIGINAL_SALT;
      cfg.activeProfile = null;
      cfg.appliedAt = new Date().toISOString();
      savePetConfig(cfg);
    } else {
...
export async function runInteractive(flags = {}) {
  ...
  // Save config (v2 schema — top-level salt for runApply compatibility)
  const config = loadPetConfig() || { version: 2, profiles: {} };
  config.version = 2;
  config.salt = result.salt;
  config.previousSalt = oldSalt;
  ...
  if (profileName.trim()) {
    const profiles = getProfiles();
    const saved = profiles[profileName.trim()];
    if (saved) {
